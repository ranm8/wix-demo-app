...
(function(window) {
    ...
    describe('Ui', function() {
        ...
        describe('onReady', function() {
            beforeEach(function() {
                angular.element.prototype.load = jasmine.createSpy();
            });

            it('should $apply the angular expression when the element has been loaded', inject(function($rootScope, $compile) {
                $rootScope.bar = jasmine.createSpy();
                $compile('<img src="foo.png" on-ready="bar()" />')($rootScope);

                expect(angular.element.prototype.load).toHaveBeenCalled();
            }));
        });

        describe('script', function() {
            it('should insert script tags with type application/json into the http cache', inject(function($rootScope, $compile, $cacheFactory) {
                $compile('<script id="hello" type="application/json">{ "foo": "bar" }</script>')($rootScope);
                expect($cacheFactory.get('$http').get('hello')).toEqual([200, '{ "foo": "bar" }']);
            }));
        });
